...
class Workflow {
 ...
 async replayStep(step){
  ...
  // assert-disabled 的目标本身就是 disabled 控件，不能走下面的通用查找
  // （通用查找含 !c.disabled 过滤，会永远匹配不到 →「没有可操作的复现控件」）
  if(step.kind==='assert-disabled'){
   const snap=await this.snapshot();
   const matches=snap.controls.filter(c=>c.name===step.target.name&&c.role===step.target.role);
   if(!matches.length)throw Error('页面上没有「'+step.target.name+'」'+step.target.role+'控件，无法断言禁用态');
   if(!matches.some(c=>c.disabled))throw Error('「'+step.target.name+'」控件未被禁用（表单必填项为空时未阻止保存）');
   this.record(step);return;
  }
  ...
  if(target.scope==='owned-row'){
   ...
   // 非 table 布局（卡片列表等）退化为文本检查：页面上能看到记录名即可继续
   if(!await rows.count()&&!(await this.page.getByText(this.currentName,{exact:false}).first().isVisible().catch(()=>false)))throw Error('找不到本轮独立记录：'+this.currentName);
  }
  let snap=await this.snapshot();
  ...
  // 删除确认被接受后 DELETE 请求异步发出，立刻 reload 会把它掐死（与 generate 侧同款陷阱）
  if(step.purpose==='delete')await this.page.waitForTimeout(1200);
 }
 async replay(file){
  const results=[];
