...
export async function openBrowser(config) {
  ...
  // ⚠️ 本项目 token 存 sessionStorage，而 Playwright storageState 不含 sessionStorage ——
  // 只靠 storageState 每次回放仍要重新登录，连续 4-5 次就撞后端登录限流（429）。
  // 因此登录成功后 runner 会另存 <hash>.session.json 快照；此处创建 context 时用
  // addInitScript 在每次导航前注入，实现跨回放免登录。
  ...
  const sessionFile = authFile ? authFile.replace(/\.json$/, '.session.json') : '';
  ...
  if (sessionFile && existsSync(sessionFile)) {
    try {
      const snapshot = JSON.parse(readFileSync(sessionFile, 'utf-8'));
      await page.addInitScript((s) => {
        try { for (const [k, v] of Object.entries(s)) sessionStorage.setItem(k, v); } catch {}
      }, snapshot);
    } catch { /* 快照损坏则忽略，走正常登录流程 */ }
  }
