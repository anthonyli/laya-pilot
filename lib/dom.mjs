...
export async function observe(page){
 ...
 for(let fi=0;fi<frames.length;fi++){
  const items=await frames[fi].evaluate(({nonce,fi,attr})=>{
   ...
   // div.fixed.inset-0.z-50：手写全屏遮罩容器（无 ARIA、无 dialog 语义），内容在其内部。
   // ⚠️ 若目标页面存在非模态的全屏 fixed z-50 容器（如 toast 容器），会把快照误限定到它。
   const modals=roots.flatMap(root=>[...root.querySelectorAll('dialog[open],[role="dialog"],[role="alertdialog"],[data-slot="dialog-content"],[data-slot="alert-dialog-content"],div.fixed.inset-0.z-50,.ant-drawer-open .ant-drawer-content,.el-dialog')]).filter(visible);
   ...
   return elements.slice(0,800).map((e,i)=>{
    ...
    // 手写表单（无 ant/el 包装）的 label 是 input 的前驱兄弟（或在前驱兄弟容器里），
    // e.labels 对这种结构不生效，需要沿 previousElementSibling 向前找最近的 label
    let sibLabel='';{let n=e.previousElementSibling;while(n){if(/^label$/i.test(n.tagName)){sibLabel=text(n);break;}n=n.previousElementSibling;}if(!sibLabel){n=e.parentElement?.previousElementSibling||null;while(n){if(/^label$/i.test(n.tagName)){sibLabel=text(n);break;}n=n.previousElementSibling;}}}
    ...
    const name=(tooltipAction||tooltip||e.getAttribute('aria-label')||labelled||labels||parentLabel||sibLabel||e.getAttribute('placeholder')||e.getAttribute('title')||ownText||selectHint||e.getAttribute('name')||'').replace(/^\*\s*/,'').slice(0,130);
    ...
    // 非表格布局（卡片列表）：取同时包含本控件与至少 2 个可交互元素的最小祖先容器，
    // 其文本作为行上下文，供 owned-row（按记录名归属）匹配使用
    let cardCtx='';
    if(!row){let n=e.parentElement;
     while(n&&n!==document.body){const t=text(n);
      // ≥30 字符排除「编辑 删除」这类纯按钮组容器，确保上下文含记录标识
      if(n.querySelectorAll('button,a[href],[role="button"]').length>=2&&t.length>=30&&t.length<260){cardCtx=t;break;}
      n=n.parentElement;}}
    const context=row?rowText.slice(0,500):(cardCtx||text(group?.querySelector('legend,h1,h2,h3'))).slice(0,500);
...
export async function target(page,laya,intent,{kind='click',value,meta={},rowKey,allowDisabled=false,minProbability=.55,minMargin=.12}={}){
 let snap=await observe(page);
